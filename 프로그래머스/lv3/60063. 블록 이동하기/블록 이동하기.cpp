...
// Key point: 현재 로봇이 차지하고 있는 두 개의 블록 중 목적지와 가까운 블록을 기준점으로 하여 이동 or 회전한다.
// -> 두 개의 좌표를 모두 관리할 필요 없이, 기준점과 로봇의 방향을 저장하여 관리
// BFS 탐색을 통해 최소 시간을 구한다. BFS를 위해 queue에 넣을 때 단순히 특정 좌표를 방문했는지 여부를 체크하는 게 아니라
// 특정 좌표에서 특정 방향으로의 탐색은 실시하였다를 3차원 배열로 나타내준다. (visit[a][b][direction])
...
int BFS(vector<vector<int>>& board)
{
    ...
    while (Q.empty() == 0)
    {
        ...
        if (Dir == 0 || Dir == 1) // 가로로 세워진 로봇일 경우의 회전 처리
        {
            int cx = x - 1;
            int cxx = xx - 1;
